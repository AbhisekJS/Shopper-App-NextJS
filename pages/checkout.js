...
import { ORDER_SET } from '../components/constants';
...
function Checkout({commercePublicKey}) {
  ...
  const handleCaptureCheckout = async () => {
    ...
    try {
        console.log(1)
        const myOrder = await commerce.checkout.capture(
        checkoutToken.id,
        orderData
      );
      dispatch({ type: ORDER_SET, payload: 14 });
      localStorage.setItem('order_receipt', JSON.stringify(myOrder));
      await refreshCart();
  ...
  return (
    <Layout title="Checkout" commercePublicKey={commercePublicKey}>
